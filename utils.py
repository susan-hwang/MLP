...
def load_mnist_trainval():
    ...
    datalen = len(data)
    data80 = int(datalen*0.8)
    train_data = data[:data80]
    train_label = label[:data80]
    val_data = data[data80:]
    val_label =label[data80:]
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    for i in range(0, len(data),batch_size):
        if i+batch_size>len(data):
            # print (i,batch_size,len(data)-i)
            batched_data.append(data[i:])
            batched_label.append(label[i:])
            
            # print(len(batched_data[-2]), len(batched_data[-1]))
            # print(len(batched_label[-2]), len(batched_label[-1]))
        else:
            # print(i,i+batch_size)
            # print(data[i].shape)
            batched_data.append(data[i:i+batch_size])
            batched_label.append(label[i:i+batch_size])
    # print(type(batched_data), type(batched_data[0]))       
    ...
    # print(batched_data.shape, batched_data[0].shape,batched_data[0][0].shape)
    # print(len(batched_label), len(batched_label[0]),batched_label[0][0])
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history, model_name):
    """
    ...
    valid_loss = np.array(valid_loss_history)
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig(model_name + '_loss_curve.jpg')

    plt.figure(2)

    train_acc = np.array(train_acc_history)
    valid_acc = np.array(valid_acc_history)
    ...
    plt.savefig(model_name + '_accuracy_curve.jpg')
